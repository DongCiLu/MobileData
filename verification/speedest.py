...
def calc_dist(compressed_session, boundaries):
...
def speed_estimation(compressed_session, rules):
    ''' estimate the speed
    ...
    compressed_session = raw_speed_estimation(compressed_session)
    # compressed_session = speed_consistency(compressed_session, rules)
    return compressed_session

def raw_speed_estimation(compressed_session):
    ''' estimate the speed use real boundaries (adjacent tower)

    speed is a range [dist_lb / loose_duration, dist_est / ((loose_duration + strict_duration) / 2)]
    ...
    for index, agg_record in enumerate(compressed_session):
        if agg_record.io[0] != None and \
                agg_record.jump_dist[0] == None:
            if buff_begin != None:
                buff_end = index
                dist_est_sum = 0
                dist_lb_sum = 0
                for buff_ix in range(buff_begin, buff_end):
                    # use same jump for est and lb, might not accurate
...
def speed_groundtruth(compressed_session, gtfn):
    start_time = None
    cs_index = 0
    agg_record = compressed_session[cs_index]
    agg_record.dist_gt = 0
    last_pos = None
    start_pos = None
    for subdir, dirs, files in os.walk(gtfn):
        for f in files:
            ...
            with open(fn) as f:
                for l in f:
                    ...
                    if start_pos == None:
                        start_pos = pos
                    dist = coordinate_dist(last_pos, pos)
                    last_pos = pos
                    # we dont care the first agg_record.
                    if time < agg_record.duration[0]:
                        continue
                    elif time > agg_record.duration[1]:
                        agg_record.speed_gt = agg_record.dist_gt / \
                                (agg_record.duration[1] - \
                                agg_record.duration[0]) * 3600 # km/h
                        agg_record.dist_gt_alt = \
                                coordinate_dist(pos, start_pos)
                        agg_record.speed_gt_alt = \
                                agg_record.dist_gt_alt / \
                                (agg_record.duration[1] - \
                                agg_record.duration[0]) * 3600 # km/h
                        cs_index += 1
                        if cs_index == len(compressed_session):
                            break
                        agg_record = compressed_session[cs_index]
                        time_limit = agg_record.duration[1]
                        agg_record.dist_gt = 0
                        agg_record.dist_gt_alt = 0
                        start_pos = pos
                    else:
                        agg_record.dist_gt += dist
